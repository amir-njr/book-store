...
const UserCard = ({ name, email, id, createdAt }) => {
  ...
  const submitHandler = async (e) => {
    ...
    try {
      const res = await fetch(UserBaseUrl(`update/${id}`), {
        method: "PATCH",
    ...
    <table className="w-full bg-white rounded-b-xl shadow-xl relative">
      <thead>
        <tr className="flex bg-slate-500">
          <th className="basis-2/12">نام</th>
          <th className="basis-2/12">ایمیل</th>
          <th className="basis-2/12">نقش</th>
